...
- **MoR + DS-LoRA**, where top-1 routed low-rank adapters are attached at selected recursion depths, defaulting to the final recursion
...
MoR assigns token-wise recursion depth with a continue/exit router while preserving dense self-attention. DS-LoRA keeps that routing behavior intact and adds a sparse low-rank branch only at high-leverage recursion depths. In the default configuration, adapters are placed at the final recursion:
...
- The default config follows the paper's final-depth insertion principle; ablations can be run by changing `lora_depth_start`, `lora_num`, and `lora_rank`.
